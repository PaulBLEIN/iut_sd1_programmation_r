#Exercice 1
df <- read.csv("L:/BUT/SD/Promo 2023/kcostisor/NBA2014_2015.csv", sep = ",",
               header = TRUE, dec = ".")
nrow(df)
ncol(df)
colnames(df)
...
#Exerice 2
 ...
 sd(df$SHOT_CLOCK)
     ...
     #les quartiles
     quantile(df$SHOT_CLOCK, probs = seq(0, 1, by = 0.25), na.rm = TRUE)
     #les déciles
     quantile(df$CLOSE_DEF_DIST, probs = seq(0, 1, by = 0.1), na.rm = TRUE)
...
#Exercice 3
       ...
       df4 <- df4[order(df4$SHOT_DIST, decreasing = TRUE), ]
       df4 <- df4[1:100, ]
       ...
       df_total_tri <- df_total[order(-df_total$PTS_MARQUES), ] # Tri décroissant avec le signe moins (-)
       df_top5 <- df_total_tri[1:5, ] # Sélection des 5 premiers joueurs
...
#Exercice4
       ...
       build_graph <- function(une_colonne, nom_colonne) {
         if(is.numeric(une_colonne)) {
           ...
           print(barplot(tri, main = nom_colonne, las = 2)) # las = 2 pour faire tourner les étiquettes si nécessaire
         }
       }
